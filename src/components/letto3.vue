...
<template>
  <div class="newlists" id="newlists">
        <h2 class="tit mt10" v-for="(value, key) in newList"  v-if=" key==0 "><i class="iconfont icon-pl5"></i><span>排例5</span></h2>
        <ul class="nlist npad">
            <li v-for="(value, key) in newList">
                <div class="bt">第 {{value.expect}} 期开奖号码</div>
                <div class="hm">
                    <ul class="red">
  ...
  methods:{
      ...
      newVue:function(num){
        ...
        axios.interceptors.response.use(response => {
          ...
          var url=window.encodeURIComponent("f.apiplus.net/pl5-20.json");
            ...
            .then(res => {
              ...
              if(str.indexOf('请求频率过')>-1){
                 res.data = sessionStorage.getItem('pl5');
              }else{
                sessionStorage.setItem('pl5',res.data);
              }
...
.hm{text-align: left; overflow: hidden;float: left;}
.hm .red {
    float: right;
    margin-left: 0.10rem;
